...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((resolve, reject) => {
    if (isPositiveAnswer === true) {
      const result = 'Hooray!!! She said "Yes"!';
      resolve(result);
    } else if (isPositiveAnswer === false) {
      const result = 'Oh no, she said "No".';
      resolve(result);
    } else {
      const result = new Error('Wrong parameter is passed! Ask her again.');
      reject(result);
    }
  });
}
...
function chainPromises(array, action) {
  ...
  array.forEach((promise) => Promise.resolve(promise)
    .then((value) => resolved.push(value))
    .catch(() => {}));

  const result = Promise.resolve(resolved).then(() => resolved.reduce(action));
